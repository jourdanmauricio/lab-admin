...
function refreshToken() {
  ...
  return import_axios.default.post("https://api.mercadolibre.com/oauth/token", JSON.stringify(data));
}
axiosAPI.interceptors.response.use(function(response) {
  ...
  if (err.response) {
    ...
    if (err.response.status === 401 && !originalConfig._retry) {
      ...
      try {
        const userStore = (0, import_index_7bf676ca.g)(import_stores_c1931941.c);
        ...
        const user = await import_Api_a77dc7b1.A.put(`/usersml/${userStore.userMl.id}`, data);
