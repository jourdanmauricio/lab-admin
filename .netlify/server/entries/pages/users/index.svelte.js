...
const Users = (0, import_index_9af3cff5.c)(($$result, $$props, $$bindings, slots) => {
  ...
  async function loadData() {
    try {
      isLoading = true;
      let url = `${import_variables_09701ff1.v.basePath}/users?limit=${pagination.limit}&offset=${pagination.offset}`;
      if (search)
        ;
  ...
  do {
    $$settled = true;
    $$rendered = `${isLoading ? `${(0, import_index_9af3cff5.v)(import_Spinner_0a5d3b24.S, "Spinner").$$render($$result, {}, {}, {})}` : ``}
...
${!$credentials.role ? `` : `${$credentials.role !== "superadmin" ? `<h1>Acceso no autorizado</h1>` : `<caption class="${"flex items-center justify-between flex-col sm:flex-row bg-secondaryColor text-white min-w-[320px] border px-10 py-1 border-gray-900 "}"><div class="${"flex"}"><i class="${"material-icons text-green-700 active:text-gray-300 cursor-pointer"}">file_download</i>
      <span class="${"text-lg ml-1"}">Usuarios: ${(0, import_index_9af3cff5.e)(pagination.total)}</span></div>
    <div class="${"flex items-center"}"><input class="${"text-black w-[250px] pl-1 svelte-jjp6dp"}" name="${"search"}" type="${"search"}"${(0, import_index_9af3cff5.h)("value", search, 0)}>
      </div>
  ...
  <table id="${"tblData"}" class="${"responsive-table"}"><thead><tr><th>Id</th>
    ...
    <tbody>${(0, import_index_9af3cff5.b)(users, (user) => {
      return `<tr><td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.id)}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.email)}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.customer ? user.customer.name : "")}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.customer ? user.customer.lastName : "")}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.role)}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.customer ? user.customer.phone : "")}</td>
          <td class="${"svelte-jjp6dp"}">${(0, import_index_9af3cff5.e)(user.customer ? user.customer.documentNumber : "")}</td>
          <td class="${"svelte-jjp6dp"}"><button${(0, import_index_9af3cff5.h)("id", user.id, 0)}><i class="${"material-icons"}" style="${"color: red"}">delete</i></button></td>
        </tr>`;
    })}</tbody></table>
...
${(0, import_index_9af3cff5.v)(import_Modal2_1c1ca4f8.M, "Modal2").$$render($$result, { this: modalDelete }, {
      ...
      default: () => {
        return `<h2 class="${"text-2xl text-center"}">Eliminar usuario ${(0, import_index_9af3cff5.e)(currentUser.customer ? currentUser.customer.name : "")}
    ${(0, import_index_9af3cff5.e)(currentUser.customer ? currentUser.customer.lastName : "")}</h2>
  ...
  <p>${(0, import_index_9af3cff5.e)(currentUser.email)}</p>
  <p>Role: ${(0, import_index_9af3cff5.e)(currentUser.role)}</p>
...
${(0, import_index_9af3cff5.v)(import_Modal2_1c1ca4f8.M, "Modal2").$$render($$result, { this: modalCreate }, {
    ...
    <form><div class="${"relative mt-10"}"><input class="${"input-oval min-w-[250px] svelte-jjp6dp"}" type="${"email"}" name="${"email"}" required="${"required"}">
      ...
      <div class="${"relative mt-10"}"><select class="${"input-oval"}" name="${"role"}" id="${"role"}"><option value="${"admin"}">Admin</option><option value="${"customer"}">Customer</option><option value="${"seller"}">Seller</option></select>
        ...
        ${errors.role ? `<p class="${"error"}"><small style="${"color: red"}">${(0, import_index_9af3cff5.e)(errors.role)}</small></p>` : ``}</div>
