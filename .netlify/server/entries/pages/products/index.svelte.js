...
const getLocalProducts = async (limit, offset, search) => {
  try {
    let paramSearch = !search ? "" : `&q=${search}`;
    const products = await import_Api_a77dc7b1.A.get(
...
const ModalDelProd = (0, import_index_7bf676ca.c)(
  ($$result, $$props, $$bindings, slots) => {
    let { currentProd } = $$props;
    let { hide } = $$props;
    if (
      $$props.currentProd === void 0 &&
      $$bindings.currentProd &&
      currentProd !== void 0
    )
      $$bindings.currentProd(currentProd);
    if ($$props.hide === void 0 && $$bindings.hide && hide !== void 0)
      $$bindings.hide(hide);
    return `<h2 class="${"text-xl text-center"}">Eliminar producto - ${(0,
    import_index_7bf676ca.e)(currentProd.name)}</h2>
...
const Products = (0, import_index_7bf676ca.c)(
  ($$result, $$props, $$bindings, slots) => {
    ...
    <span class="${"text-lg ml-5"}">Productos: ${(0, import_index_7bf676ca.e)(
        pagination.total
      )}</span></div>
  <div class="${"flex items-center"}"><input class="${"text-black w-[250px] pl-1"}" name="${"search"}" type="${"search"}"${(0,
      import_index_7bf676ca.i)("value", search, 0)}></div>
  </caption>
...
<table id="${"tblData"}" class="${"responsive-table"}"><thead><tr><th>ID</th>
  ...
  <tbody>${(0, import_index_7bf676ca.b)(products, (product) => {
    return `<tr><td>${(0, import_index_7bf676ca.e)(product.id)}</td>
        <td><img${(0, import_index_7bf676ca.i)(
          "src",
          product.thumbnail,
          0
        )} alt="${""}"></td>
        ...
        <td><button${(0, import_index_7bf676ca.i)(
          "id",
          product.id,
          0
  ...
  },
  {
    default: () => {
      return `${(0, import_index_7bf676ca.v)(
        ModalDelProd,
        "ModalDelProd"
      ).$$render($$result, { currentProd, hide }, {}, {})}`;
    },
  }
)}`;
    } while (!$$settled);
